...
The following requirements must be met in Dash Platform data contracts:
 ...
 - Contested unique indexes resolve ownership through governance. Such an index must set `"unique": true` and include a `"contested"` object (never a boolean) with `"resolution": 0`, and may optionally include a valid `"fieldMatches"` array and a `"description"` string. The document type that owns a contested index MUST also set `"documentsMutable": false` at the document-type level (contested indexes are only permitted on immutable document types), and that document type must not declare any other non-contested `"unique": true` index.
"#;
